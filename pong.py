...
if sys.version_info < (3,5):
    print("This game requires at least version 3.5 of Python. Please Download it from python.org")
    sys.exit()
...
PLAYER_SPEED = 6
MAX_AI_SPEED = 6
#change ai speed for difficulty switch
...
class Bat(Actor):
    ...
    def ai(self):
        ...
        target_y_2 = game.ball.y + game.ai.ai_offsetweight1 = min(1, x_distance / HALF_WIDTH)
        weight2 = 1 - weight1
        target_y = (weight * target_y_1) + (weight2 * target_y_2)
...
class Game:
    ...
    def update(self):
        ...
        if self.ball.out():
            ...
            losing player = 1 - scoring_player
    ...
    def play_sound(self, name, count=1):
        if self.bats[0].move_func != self.bats[0].ai:
            try:
                gatattr(sounds, name + str(random.randint(0, count - 1))).play()
            except:
                pass
...
try:
    pygame.mixer.quit()
    pygaem.mixer.init(44100, -16, 2, 1024)
